...
{    
    ...
    new_head->value = (char *) malloc((strlen(s) + 1) * sizeof(char));
    if (new_head->value == NULL) return false;
...
bool q_insert_tail(queue_t *q, char *s)
{
    ...
    new_tail->value = (char *)malloc((strlen(s) + 1) * sizeof(char));
    if(new_tail->value == NULL) return false;
    ...
    if (q->size == 1){
        q->head = new_tail;
        q->tail = new_tail;
    } else if (q->size > 1) { /* Else, append tail.*/
        q->tail->next = new_tail;
        q->tail = new_tail;
    } else {
        printf("You queue size is %d it's not a legal input.", q->size);
    }
    return true;
}
...
bool q_remove_head(queue_t *q, char *sp, size_t bufsize)
{
    ...
    if (sp != NULL){
        strncpy(sp, want_free->value, bufsize-1);
        if (sizeof(want_free->value) > bufsize-1){ /* If length bigger than bufsize, then mean cliped, so we need pad "\0"*/
            sp[bufsize - 1] = '\0';
        }
    }
    free(want_free->value);
